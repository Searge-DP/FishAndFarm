...
public class EntityIronFishHook extends EntityFishHook implements IEntityAdditionalSpawnData {
    ...
    @SideOnly(Side.CLIENT)
    public EntityIronFishHook(World world, double x, double y, double z, EntityPlayer entityPlayer)
    {
        this(world);
    ...
    public EntityIronFishHook(World world, EntityPlayer entityPlayer)
    {
        super(world);
        this.xTile = -1;
        this.yTile = -1;
        ...
        this.setSize(0.25F, 0.25F);
        this.setLocationAndAngles(entityPlayer.posX, entityPlayer.posY + 1.62D - (double)entityPlayer.yOffset, entityPlayer.posZ, entityPlayer.rotationYaw, entityPlayer.rotationPitch);
    ...
    /**
     * Calculates the new velocity using old velocity and multipliers?
     * 
     * @param xMotion The old x motion
     * @param yMotion The old y motion
     * @param zMotion The old z motion
     * @param par7 Some kind of multiplier applied to the motion
     * @param par8 Also a kind of multiplier
     */
    public void calculateVelocity(double xMotion, double yMotion, double zMotion, float par7, float par8)
    {
        float f2 = MathHelper.sqrt_double(xMotion * xMotion + yMotion * yMotion + zMotion * zMotion);
        xMotion /= (double)f2;
        yMotion /= (double)f2;
        zMotion /= (double)f2;
        xMotion += this.rand.nextGaussian() * 0.007499999832361937D * (double)par8;
        yMotion += this.rand.nextGaussian() * 0.007499999832361937D * (double)par8;
        zMotion += this.rand.nextGaussian() * 0.007499999832361937D * (double)par8;
        xMotion *= (double)par7;
        yMotion *= (double)par7;
        zMotion *= (double)par7;
        ...
        float f3 = MathHelper.sqrt_double(xMotion * xMotion + zMotion * zMotion);
        this.prevRotationYaw = this.rotationYaw = (float)(Math.atan2(xMotion, zMotion) * 180.0D / Math.PI);
        this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(yMotion, (double)f3) * 180.0D / Math.PI);
     ...
     * @param rotationIncrement The new rotation increment? 
     */
    @SideOnly(Side.CLIENT)
    public void setPositionAndRotation2(double posX, double posY, double posZ, float yaw, float pitch, int rotationIncrement)
    {
        this.fishX = posX;
        this.fishY = posY;
        this.fishZ = posZ;
        ...
        this.fishPosRotationIncrements = rotationIncrement;
     ...
     * 
     * @param xMotion The new x motion
     * @param yMotion The new y motion
     * @param zMotion The new z motion
     */
    @SideOnly(Side.CLIENT)
    ...
    public void onUpdate()
    {
        ...
        else
        {
            if (!this.worldObj.isRemote)
            {
                ...
                if (this.angler.isDead || !this.angler.isEntityAlive() || itemstack == null || itemstack.getItem() != FishAndFarm.ironFishingRod || this.getDistanceSqToEntity(this.angler) > 1024.0D)
                {
                    this.setDead();
	...
	public void readSpawnData(ByteArrayDataInput data) {
		Entity entity = this.worldObj.getEntityByID(data.readInt());
		if (entity instanceof EntityPlayer) {
			this.angler = (EntityPlayer)entity;
			((EntityPlayer)entity).fishEntity = this;
		}
	}
}
