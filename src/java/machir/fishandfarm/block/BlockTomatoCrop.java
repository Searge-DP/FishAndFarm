...
public class BlockTomatoCrop extends BlockCrop {
	private static final int TOMATO_DAMAGE = 3;
	private static final int SEED_DAMAGE = 1;
    ...
    public int damageDropped(int damage)
    {
    	// If the crop is on it's last stage, drop the item
    	// If not, drop a seed
        return damage == this.maxGrowthStage ? this.TOMATO_DAMAGE : this.SEED_DAMAGE;
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ)
    {
    	...
    	if (world.getBlockMetadata(x, y, z) == this.maxGrowthStage) {
    		...
    		world.setBlockMetadataWithNotify(x, y, z, this.maxGrowthStage - 2, 2);
    		return true;
    	}
        return false;
    }
